...
class TestGarageController:
    ...
    def test_garage_door_status__should_call_is_garage_door_open_status(self, mock_thread, mock_gpio):
        get_status(self.JWT_TOKEN)

        mock_gpio.is_garage_open.assert_called()

    @patch('svc.controllers.garage_door_controller.datetime')
    def test_garage_door_status__should_return_status(self, mock_date, mock_thread, mock_gpio):
        now = datetime.now()
    ...
    def test_garage_door_status__should_return_open_garage_status_date_when_active_thread(self, mock_thread, mock_gpio):
        self.STATE.ACTIVE_THREAD = MyThread(Event(), print, Automation.TIME.THIRTY_SECONDS)
        now = datetime.now()
        self.STATE.STATUS = False
    ...
    def test_garage_door_status__should_return_close_garage_status_date_when_active_thread(self, mock_thread, mock_gpio):
        self.STATE.ACTIVE_THREAD = MyThread(Event(), print, Automation.TIME.THIRTY_SECONDS)
        now = datetime.now()
    ...
    def test_garage_door_status__should_call_is_jwt_valid(self, mock_validate, mock_thread, mock_gpio):
        get_status(self.JWT_TOKEN)

        mock_validate.assert_called_with(self.JWT_TOKEN)

    def test_update_garage_door_state__should_return_response_(self, mock_thread, mock_gpio):
    ...
    def test_update_garage_door_state__should_call_update_gpio(self, mock_thread, mock_gpio):
    ...
    @patch('svc.controllers.garage_door_controller.is_jwt_valid')
    def test_toggle_garage_door_state__should_validate_bearer_token(self, mock_jwt, mock_thread, mock_gpio):
        toggle_garage_door_state(self.JWT_TOKEN)

        mock_jwt.assert_called_with(self.JWT_TOKEN)

    def test_toggle_garage_door_state__should_call_gpio_pins(self, mock_thread, mock_gpio):
        toggle_garage_door_state(self.JWT_TOKEN)
